...
- **Geometry features**: MediaPipe facial landmarks, distances, and angles
...
3. Add `mediapipe_features.py` with `extract_geometry_features(image)`.
4. Run `main.py` to execute experiments 1-7 in order. This trains and evaluates
   each classifier on the dataset split in the manifest.
5. Compare the printed accuracy tables and the generated `summary_*.csv`,
...
```bash
python3 image_inference.py path/to/image.jpg --model outputs/experiments/07_all_features/model.joblib
```
...
- `feature_extraction.py`
  ...
  - Expects a future `mediapipe_features.py` file with
    `extract_geometry_features(image)` when geometry experiments are used.
...
- `test_frequency.py`
  ...
  - Useful for checking that `frequency.py` behaves consistently.

### Experiment Entry Files

The files in `experiments/` are intentionally small wrappers around
`experiment_runner.py`. Their purpose is to give each ablation experiment a clear
command and output folder while keeping the actual training logic in one shared
place.
...
- `experiments/experiment_02_frequency.py`
  ...
  - Uses DCT features from `frequency.py`.

- `experiments/experiment_03_geometry.py`
  - Runs experiment 3: geometry features only.
  - Requires the future `mediapipe_features.py` extractor.
...
### Facial Embedding Reference Folder

The `facial_embedding/deepfakes/` folder contains the earlier video-based ArcFace
experiment that inspired the image-only `arcface_embedding.py` module.

- `facial_embedding/deepfakes/README.md`
  - Documentation for the original latent-space outlier detection notebook.

- `facial_embedding/deepfakes/requirements.txt`
  - Dependencies for the original notebook experiment.

- `facial_embedding/deepfakes/deepfake_detection.ipynb`
...
- `facial_embedding/deepfakes/deepfake_detection_executed.ipynb`
  - Executed version of the same notebook with saved outputs.

- `facial_embedding/deepfakes/outputs/manipulated.mp4`
  - Generated sample manipulated video from the notebook experiment.

- `facial_embedding/deepfakes/outputs/manipulated_annotated.mp4`
  - Annotated version of the manipulated video with face boxes, keypoints, and
    detection labels.

- `facial_embedding/deepfakes/outputs/umap.png`
  - UMAP visualization of ArcFace embeddings from the notebook.

- `facial_embedding/deepfakes/outputs/histogram.png`
...
project comparison does not require `evaluate_model.py`.
...
- Avoid naming the geometry file `mediapipe.py`; use `mediapipe_features.py` so it
  does not shadow the installed `mediapipe` package.
